...
class Rptconpre extends CI_Controller {
    var $_arr_Sesion;
    ...
    public function __construct() {
        ...
        $this->lang->load('rptconpre');
        $this->load->helper('form');
        $this->load->helper('date');
        $this->load->library('form_validation');
        $this->load->model('rptconpre_m');
    }
    public function index() {
        ...
        $arrSesion['title'] = lang('rptconpre.title');
        $arrMenu['mGroup'] = 'm_consultas';
        $arrMenu['mOption'] = 'm_consultas_07';
        ...
        $this->load->view('frontend/reporteconpre/rptconpre_list_v',$arraySesion);
    ...
    function get_proyectos(){
        ...
        $objmovimiento = new $this->rptconpre_m;
    ...
    function get_conceptos(){
        if ($this->seguridad->sec_class(__METHOD__)) return;
        $arrParam = array(null,2);
        $objtransaccion = new $this->rptconpre_m;
    ...
    function get_cajas(){
        ...
        $arrParam = array(null,2);
        $objtransaccion = new $this->rptconpre_m;
        $str = $objtransaccion->get_cajas($arrParam);
        return json_encode($str);
    }
    ...
    public function view_movimiento(){
        ...
        $result = $this->rptconpre_m->get_transaccion($data);
        ...
        if (count($result) == 0) {
            ...
            $arrSesion['arrTipo'] = $this->rptconpre_m->get_tipo();
            $arrSesion['arrCaja'] = $this->rptconpre_m->get_caja();
            $arrSesion['arrSproyecto'] = $this->rptconpre_m->get_sproyecto();
            $arrSesion['arrProyecto'] = $this->rptconpre_m->get_proyecto();
            $arrSesion['arrSConcepto'] = $this->rptconpre_m->get_sconcepto();
            $arrSesion['arrConcepto'] = $this->rptconpre_m->get_concepto();
            $arrSesion['arrEmpresa'] = $this->rptconpre_m->get_empresa();
            $this->load->view('frontend/reportemov/reportemov_view_v', $arrSesion);
        }
    }
    
    public function load_reporte($proyectoID,$conceptoID,$c_codigo,$fecha_ini,$fecha_fin){
        if ($this->seguridad->sec_class(__METHOD__)) return;
        $arrRpt = array(
            'arrPry'     => $proyectoID,
