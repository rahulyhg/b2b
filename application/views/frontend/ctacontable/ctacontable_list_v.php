...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var Seleccione = "<?= lang('ctacon.selec')?>";
        // boton Nuevo
        $('#btnNew').on('click', (function() {
            var param = null;
            $.openModal('#btnNew', 'ctacontable/new_ctacontable', '#dlgNew', param, "<?= lang('g.msgProcesing')?>");
        }));
        ...
        $('#btnEdit').on('click', (function() {
            ...
            if ( grid.dataItem(grid.select()) != undefined){
                ...
                var param = {id : id};                
                $.openModal('#btnEdit', 'ctacontable/edit_ctacontable', '#dlgNew', param, "<?= lang('g.msgProcesing')?>");
            } else{
        ...
        $('#btnDelete').on('click', (function() {
            ...
            if ( grid.dataItem(grid.select()) != undefined){
                var id = grid.dataItem(grid.select()).id_cuenta;
                var param = {id : id};
                $.openModal('#btnDelete', 'ctacontable/remove_ctacontable', '#dlgNew', param, "<?= lang('g.msgProcesing')?>");
            } else{
                jAlert('<?= lang('valid.select') ?>', 'Caja');
            }
        }));
            ...
            transport: {
                read:  function(options){
                    $.ajax({
                        type: "POST",
                        dataType: "json",
                        url: "<?= site_url('ctacontable/get_ctacontable_list') ?>",
        ...
        $("#tbData").kendoGrid({
            ...
            columns: [ 
                ...
                { field: "c_cuenta", title: "Cuenta Contable" },
