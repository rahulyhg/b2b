...
$(document).ready(function() {
    ...
    var gridElementS = $("#tbSaldos"), gridElement = $("#tbData"),
        ...
        htmlStart = 
            ...
            '<head>' +
            '<meta charset="utf-8" />' +
            '<title>Reporte Detalle de Caja por Empresa</title>' + 
            ...
            '</style>' +
            '</head>' + 
            '<body><div><center><b>REPORTE DETALLE DE CAJA POR EMPRESA ' + $("#txt_fec_ini").val() + ' - ' + $("#txt_fec_fin").val() +
            ...
            transport: {
                read:  function(options){
                    $.ajax({
                        type: "POST",
                        dataType: "json",
                        url: "<?= site_url('rptcajaemp/get_rptcajaemp_sum') ?>",
         ...
         var dataSource = new kendo.data.DataSource({
            transport: {
                read: { 
                        type: "POST",
                        dataType: "json",
                        url: "<?= site_url('rptcajaemp/get_reportemov_list') ?>",
                        success: 
                              function (resultado){
                               options.success(resultado);
                        }
...
//                        EMPRESA:        { type: "string" },
            ...
            group: { field: "EMPRESA",
                     aggregates: [{ field: "EMPRESA", aggregate: "count" },
                                  
                                  { field: "IMPORTE", aggregate: "sum" },
                                  { field: "IMPORTE_DOL", aggregate: "sum" }]
            },
            aggregate: [{ field: "EMPRESA", aggregate: "count" },
                        { field: "EMPRESA", aggregate: "sum" },
                        { field: "IMPORTE", aggregate: "sum" },
                        { field: "IMPORTE_DOL", aggregate: "sum" }]
});
   $('#btnVer').click(function (e){
        ...
        var cb_concepto = $('#cb_concepto').data("kendoComboBox");
         ...
         var URL= "rptcajaemp/load_reporte/";
...
//                  attributes:{style:"font-size: 11px; width: px"},
//                  groupHeaderTemplate: " #=value# (Nro. de Transacciones: #=count#) "
//                },
                { field: "CAJA",
                  //width: 20,
                  title: "<div style='font-size: 12px; width: 95px'>Caja</div>",
                  attributes:{style:"font-size: 10px"}
                },
                { field: "PROYECTO",
                  //width: 20,
                  title: "<div style='font-size: 12px; width: 98px'>Proyecto</div>",
                  attributes:{style:"font-size: 11px"}
                },
                { field: "SPROYECTO",
                  //width: 20,
                  title: "<div style='font-size: 12px; width: 62px'>Subproyecto</div>",
                  attributes:{style:"font-size: 11px"}
                },
                { field: "c_concepto",
                  //width: 20,
                  title: "<div style='font-size: 12px; width: 76px'>Concepto</div>",
                  attributes:{style:"font-size: 11px"} 
                },
                { field: "c_sconcepto",
                  //width: 20,
                  title: "<div style='font-size: 12px; width: 63px;'>Subconcepto</div>",
                  attributes:{style:"font-size: 12px"}
                },
                { field: "observacion",
                  //width: 20,
                  title: "<div style='font-size: 12px; width: 155px;'>Observacion</div>" ,
                  attributes:{style:"font-size: 10px"},
                ...
                { field: "IMPORTE", 
                  //width: 20,
                  title: "<div style='font-size: 12px; width: 38px;'>Importe_S/.</div>" ,
                  format  : "{0:n2}",
                  decimals: 2, 
                  step    : 0.01,
                ...
                { field: "IMPORTE_DOL",
                  //width: 20,
                  title: "<div style='font-size: 12px; width: 39px;'>Importe_$</div>",
                  format  : "{0:n2}",
                  decimals: 2, 
                  step    : 0.01,
                  attributes:{style:"text-align:right; font-size: 13px"},
 ...
 $("#btnExportar").click(function(e) {
     ...
     result += "<table><tr><th>EMPRESA</th><th>id_transaccion</th><th>FECHA</th><th>CAJA</th><th>C.ASIGNACION</th><th>SUB C.ASIGNACION</th><th>CONCEPTO</th><th>SUBCONCEPTO<th>OBSERVACION</th><th>IMPORTE S/.</th><th>IMPORTE US$</th></tr>";
     ...
     for (var i = 0; i < data.length; i++) {
         ...
         result += "<td>";
         result += data[i].PROYECTO;
         result += "</td>";

         result += "<td>";
         result += data[i].SPROYECTO;
         result += "</td>";
