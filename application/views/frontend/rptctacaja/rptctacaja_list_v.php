...
$(document).ready(function() {
   ...
   $.populateComboBox('#cb_concepto', "<?= lang('rptmov.selectall') ?>", 'Concepto', 'id_concepto',<?php echo (isset ($jsonConceptos)) ? $jsonConceptos : 'null' ?>,1 );
    ...
    var gridElementS = $("#tbSaldos"), gridElement = $("#tbData"),
        ...
        htmlStart = 
            ...
            '<head>' +
            '<meta charset="utf-8" />' +
            '<title>Reporte Detalle de Cuenta por Caja/Banco</title>' + 
            ...
            '</style>' +
            '</head>' + 
            '<body><div><center><b>REPORTE DETALLE DE CUENTA POR CAJA/BANCO ' + $("#txt_fec_ini").val() + ' - ' + $("#txt_fec_fin").val() +
            ...
            transport: {
                read:  function(options){
                    $.ajax({
                        type: "POST",
                        dataType: "json",
                        url: "<?= site_url('rptctacaja/get_rptctacaja_sum') ?>",
         ...
         var dataSource = new kendo.data.DataSource({
            transport: {
                read: { 
                        type: "POST",
                        dataType: "json",
                        url: "<?= site_url('rptctacaja/get_reportemov_list') ?>",
                        success: 
                              function (resultado){
                               options.success(resultado);
                        }
            ...
            group: { field: "CAJA",
                     aggregates: [{ field: "CAJA", aggregate: "count" },
                                  
                                  { field: "IMPORTE", aggregate: "sum" },
                                  { field: "IMPORTE_DOL", aggregate: "sum" }]
            },
            aggregate: [{ field: "CAJA", aggregate: "count" },
                        { field: "CAJA", aggregate: "sum" },
                        { field: "IMPORTE", aggregate: "sum" },
                        { field: "IMPORTE_DOL", aggregate: "sum" }]
        });
   $('#btnVer').click(function (e){
        ...
        var cb_concepto = $('#cb_concepto').data("kendoComboBox");
         ...
         var URL= "rptctacaja/load_reporte/";
        ...
        $("#tbData").kendoGrid({
            ...
            columns: [
                ...
                { field: "CHEQUE",
                  width: 50,
                  title: "<div style='font-size: 12px'>Cheque</div>",
                  attributes:{style:"font-size: 11px"}
                },
                { field: "PROYECTO",
                  width: 100,
                  title: "<div style='font-size: 12px'>Proyecto</div>",
                  attributes:{style:"font-size: 11px"}
                },
                { field: "SPROYECTO",
                  width: 80,
                  title: "<div style='font-size: 12px'>Subproyecto</div>",
                  attributes:{style:"font-size: 11px"}
                },
                { field: "c_concepto",
                  width: 80,
                ...
                { field: "c_sconcepto",
                  width: 80,
                  title: "<div style='font-size: 12px'>Subconcepto</div>",
                  attributes:{style:"font-size: 11px"}
                },
                { field: "observacion",
                  width: 150,
                ...
                { field: "IMPORTE_DOL",
                  width: 60,
                  title: "<div style='font-size: 12px'>Importe US$</div>",
                  format  : "{0:n2}",
                  decimals: 2, 
                  step    : 0.01,
                  attributes:{style:"text-align:right; font-size: 13px"},
 ...
 $("#btnExportar").click(function(e) {
     ...
     result += "<table><tr><th>Empresa</th><th>Id</th><th>Fecha</th><th>Caja</th><th>Proyecto</th><th>Subproyecto</th><th>Concepto</th><th>Subconcepto<th>Observacion</th><th>Importe S/.</th><th>Importe US$</th></tr>";
