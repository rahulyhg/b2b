    ...
    $(objeto).submit(function(event) {  
            ...
            if (tmp.indexOf(pivot) < 0 && tmp.substring(0, 3) == 'cb_'){
                ...
                if (combobox != undefined){
                    if (combobox.text() != '' && combobox.value() != ''){
                        if (combobox.text() == combobox.value()){
                            jAlert(arrMensajes[0] + combobox.text(), adexusApp);
                            return;
                        }
                    }
                }
        ...
        $.ajax({
            ...
            success: function(resultado){
                ...
                if (resultado == ''){
                    ...
                    if(redirect != null){
                       ...
                       if(gs_idioma == 'es')  
                            jAlert("La contraseña se cambio correctamente",adexusApp);
                        else
                            jAlert("The password was changed successfully",adexusApp);
                    }
                    $('#close').trigger('click');
                }else {
                    jAlert(resultado, adexusApp);
                }
            ...
            error: function(){
                ...
                jAlert(arrMensajes[1], adexusApp);
                if (boton != ''){
                    $(boton).html(txtBoton);
                    $(boton).removeAttr("disabled");
                }
    ...
    $.ajax({
        ...
        success: function(resultado){
            if (resultado){
                ...
                jAlert (arrMensajes[2], adexusApp);
            }
            if (boton != ''){
                $(boton).html(txtBoton);
                $(boton).removeAttr("disabled");                    
        ...
        error: function(){
            ...
            jAlert(arrMensajes[3], adexusApp);
        }
    });
